...
引き継いでよいのは、確定要件、有限コードとAPIの意味、座標・幾何の定義、テスト期待値、利用許諾を確認できたfixture、プラットフォーム非依存の数式と状態ガードだけである。これらをSwift向けに再構成した`requirements.md`、`task.md`、`AGENTS.md`、`.agents/skills/`は本リポジトリで維持するが、参照元のOpenSpec・要件文書・React用skill・Web実装をそのままコピーしない。React/React DOM、Vite/TypeScript/Zod、`useReducer`実装、`getUserMedia`、`HTMLVideoElement`、Canvas、Blob、object URL、Webのframe callback、Web Worker、OpenCV.js/WASM、`object-fit`変換、Safari/page lifecycle対応、Node.js baseline API、Webテスト/build/CSS/Storybook、不採用OSSはコピーしない。backendをSwiftへ移植するタスクも作らない。
...
- [x] **T08-02 token取得、Room接続、camera publish、助言購読を接続する**
  ...
  - P0修正記録 (2026-09-01): 状態は`code_ready_unverified`。production/test commitは`8422cde`。現行backend `main`を2026-09-01 09:30 JSTに読み取り専用再取得（`a25a8542664b4bd3bfe3ff00171ea56cd373966c`）し、token routeが`listing-photo-session-<app sessionId>`をRoom名にする一方、AgentがRoom名を`GuidanceEvent.sessionId`に使うためSwiftの厳密filterが全助言を拒否する不一致を確認した。現在tokenのRoom名と完全一致するeventだけをapp sessionへ正規化し、任意の別sessionとleave/failure/reconnect前のRoom名は引き続き拒否する互換層を実装した。対応test codeは現行Room別名の受理とapp session出力、任意別名拒否、token更新後の旧Room別名拒否を含む。限定source reviewは`P0 none`、`git diff --check`とsecret static scanは成功、build/testはPhase 1方針により未実行。共有HTTPS/LiveKit設定、provider credential、backendの`capture_context`受信→`AgentRuntime.set_shot()`接続、実機publish/subscription/push、5分thermal/memory、p95、live acceptanceは残るgateで、fixture結果を代用しない。
