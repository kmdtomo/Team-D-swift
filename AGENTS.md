...
- After the initially assigned production scope and test code are committed, source-reviewed, recorded in the owned `task.md` block, and reported, stop. Do not select the next checklist item, refill the same worker with another task, or continue toward repository-wide completion unless the initial request explicitly assigned that broader range. A parent may dispatch only task IDs inside its own fixed initial scope, and each child ends after returning its bounded candidate and implementation-review data.
...
- Track work as `planned`, `in_progress`, `code_ready_unverified`, `implementation_ready`, `integration_ready`, `accepted`, or `blocked`. `[x]` begins at `code_ready_unverified`: the full task production scope and corresponding test code (or a justified `not applicable`) are committed, one bounded source review found no P0, and the implementation record names commits and pending gates. Build/test/live/device/final acceptance remain separate evidence. Commit existence, a partial slice, missing app wiring, or unresolved P0 stays `[ ]`; checkbox state is never a start gate.
...
- During the implementation phase, write the required tests but do not run `swift build`, `swift test`, `xcodebuild`, XCUITest, app launches, clean builds, or other compile/link commands. Source-only lint, schema/document validation, secret scans, and `git diff --check` are allowed when they create no build artifacts. A test is required whenever the task has automatable behavior; only genuinely docs-only or physical-only work may record `tests authored: not applicable` with a reason. Never add filler tests solely to obtain a check.
- After integration, perform one bounded source review. Only P0 blocks `[x]`; P1 through P3 may be recorded as small follow-ups but must not start another implementation or review loop. When P0 is absent, mark the task `code_ready_unverified`, check it, and commit its implementation-review record before ending the assigned scope.
...
- If verification fails, treat compiler/linker failures and required-test failures as `P0`, fix the responsible code/configuration/test, and rerun only after that change. Skip `P1` through `P3`; do not start another implementation or audit loop for them. Keep `[x]` while the task's full implementation remains committed; revert to `[ ]` only when verification proves that production scope or required test code is actually incomplete.
...
- Record Phase 1 implementation evidence under the task: status, production/test commits, full production scope, tests authored or justified `not applicable`, `P0 none`, build/test deferred, and pending integration/fixture/live/device/acceptance gates. The task-record commit is tracked by Git and must not self-reference its own SHA. Do not uncheck or rewrite pre-policy records for formatting alone; require concrete evidence of missing full production scope, missing required test code, or P0. Record Phase 2 verification separately: commit/build, test command and result, separate fixture and live configuration/result/artifact entries, device/iOS/checks/runbook or `not required`, backend version, and any remaining limitation. Final acceptance is determined from the state plus verification evidence, not from `[x]` alone.
