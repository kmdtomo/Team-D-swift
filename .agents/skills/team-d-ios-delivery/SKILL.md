...
- Do not override a child agent's model or reasoning effort by default. Spawn implementation and review children without model or reasoning overrides so they inherit the parent agent's settings. Override them only when the user explicitly requests a specific model or reasoning effort for that child.
...
- In final verification, assign one direct child as the exclusive verification-and-fix owner of one clean integrated worktree. Let it inherit the parent agent's model and reasoning settings unless the user explicitly requests an override. Other children must not write to that worktree; they may perform bounded read-only diagnosis only when the verification owner requests a specific failure analysis.
